...
def myAlgo(params, **kwargs):
    ...
    domaBuyThresh = float(params.get("domaBuyThresh", "0.0"))
    domaSellThresh = float(params.get("domaSellThresh", "0.0"))
    ...
    cmd = r'python3 -u {:s}/tutorials/src/flf/StrategyRunner.py --writeOutputToFiles=False --inputGlobPath=tests/testData/features_reinfocement_training_*.csv --evalMethod=TA_ManyTraining --taStrategy=150 --mfiPeriod={:d} --ma1Period={:d} --ma2Period={:d} --ma1Type={:d} --ma2Type={:d} --domaBuyThresh={:0.2f} --domaSellThresh={:0.2f} --priceReversalSig={:d} --indicatorReversalSig={:d} --reversalPointLookbackCount={:d} --bbPriceType={:d}'\
