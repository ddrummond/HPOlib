...
def myAlgo(params, **kwargs):
    ...
    cmd = r'python3 -u {:s}/tutorials/src/flf/StrategyRunner.py --random_state=1234 --writeOutputToFiles=False --inputGlobPath=tests/testData/features_reinfocement_training_*.csv --useSaveQSpace=True --trainingEpochs=4 --trainingBatchSize=15 --enterLongThresh={:.4f} --exitLongThresh={:.4f} --reversalSig={:d} --model=0 --layerSizeStep=1 --nb_epoch={:d} --layerCount={:d} --firstLayerOutputCount={:d} --learningRate={:.9f} --beta1={:.9f} --beta2={:.9f} --epsilon={:.9f} --rho={:.9f} --decay={:.9f} --momentum={:.9f} --nesterov={} --inputDropOutRate={:.9f} --hiddenDropOutRate={:.9f} --instanceOrder=1 --stackedModelConfigsFile=flf/stackedRegressorConfig.yaml'.format(experimentHome, enterLongThresh, exitLongThresh, reversalSig, nb_epoch, layerCount, firstLayerOutputCount, learningRate, beta1, beta2, epsilon, rho, decay, momentum, nesterov, inputDropOutRate, hiddenDropOutRate)
