...
def myAlgo(params, **kwargs):
    ...
    nb_epoch = int(params.get("nb_epoch", "4"))
    layerCount = int(params.get("layerCount", "12"))
    firstLayerOutputCount = int(params.get("firstLayerOutputCount", "123"))
    learningRate = float(params.get("learningRate", "0.0968"))
    ...
    decay = float(params.get("decay", "0.0077"))
    momentum = float(params.get("momentum", "0.1740"))
    nesterov = bool(params.get("nesterov", "True"))
    inputDropOutRate = float(params.get("inputDropOutRate", "0.1092"))
    hiddenDropOutRate = float(params.get("hiddenDropOutRate", "0.000182"))
    n_estimators = int(params.get("n_estimators", "20"))
    trainingSampleFraction = float(params.get("trainingSampleFraction", "0.50"))
    ...
    cmd = r'python3 -u {:s}/tutorials/src/flf/StrategyRunner.py --writeOutputToFiles=False --inputGlobPath=tests/testData/features_reinfocement_training_*.csv --useSaveQSpace=True --trainingEpochs=15 --trainingBatchSize=15 --enterLongThresh={:.4f} --exitLongThresh={:.4f} --reversalSig={:d} --model=5 --layerSizeStep=1 --nb_epoch={:d} --layerCount={:d} --firstLayerOutputCount={:d} --learningRate={:.9f} --beta1={:.9f} --beta2={:.9f} --epsilon={:.9f} --rho={:.9f} --decay={:.9f} --momentum={:.9f} --nesterov={} --inputDropOutRate={:.9f} --hiddenDropOutRate={:.9f} --instanceOrder=1 --n_estimators={:d} --trainingSampleWithReplacement=False --trainingSampleFraction={:.9f}'.format(experimentHome, enterLongThresh, exitLongThresh, reversalSig, nb_epoch, layerCount, firstLayerOutputCount, learningRate, beta1, beta2, epsilon, rho, decay, momentum, nesterov, inputDropOutRate, hiddenDropOutRate, n_estimators, trainingSampleFraction)
